                            ...
                            @foreach ($hsCodeCounts as $count_hs_code => $count) 
                                ...
                                @php
                                    ...
                                    if ($hs_code && $desc && $hs_code !== ' ' && $desc !== ' ') {
                                        ...
                                        $country_url =  route('searchfiltertwo', ['country'=>$country,'type' => $type, 'role' => $role,'filterby2' => 'country','filterby'=>$product,'filter'=>$desc,'filterby1'=>$hscode,'filterdata' => $res_hs_code, 'filterdata1' => $destination_country]);
                                    } elseif ($hs_code!==' ' && $hs_code!==null) {
                                        # code...
                                        $country_url = route('search-filter', ['country'=>$country,'type' => $type, 'role' => $role,'search'=>$search,'base_search' => $base_hs_code, 'filterby' => 'country', 'filterdata' => $destination_country]);
                                    } elseif ($desc!==' ' && $desc !== null) {
                                        # code...
                                        $country = str_ireplace(" ", "-", $country);
                                  
                                        $country_url = route('search-filter', ['country'=>$country,'type' => $type, 'role' => $role,'search'=>$search,'base_search' => $base_desc??"null", 'filterby' => 'country', 'filterdata' => $destination_country??"null"]);
                                    }
                                    // Port Url
                                    // Port Url
                                    if ($hs_code && $desc && $hs_code !== ' ' && $desc !== ' ') {
                ...
                @foreach ($Mresult as $Mresult)
                    @php
                        ...
                        if ($hs_code) {
                            # code...
                            $unloading_port = str_ireplace(" ", "-", $unloading_port);
                            $port_url = route('search-filter', ['country'=>$country,'type' => $type, 'role' => $role,'search'=>$search,'base_search' => $base_hs_code, 'filterby' => 'port', 'filterdata' => $unloading_port??"Null"]);
                        } else {
                            # code...
                            $unloading_port = str_ireplace(" ", "-", $unloading_port);
                            $port_url = route('search-filter', ['country'=>$country,'type' => $type, 'role' => $role,'search'=>$search,'base_search' => $base_desc, 'filterby' => 'port', 'filterdata' => $unloading_port??'null']);
                        }
                    @endphp
